...
# Load data
features <- read_csv("synthetic data/tourists_features.csv")
features$...1 <- NULL

# ----- LASSO variable selection per KKR for D8R1 -----
lasso_vars_per_kkr_d8r1 <- list()
...
for(kkr_val in unique(features$KKR)) {
  ...
  x <- df_kkr %>%
    select(-c(D8R1, REGON, OKRES, RO, WON, POW, GMN, KKR, date))
  ...
  lasso_vars_per_kkr_d8r1[[as.character(kkr_val)]] <- selected_vars
}

# ----- Build linear regression models for each KKR -----
lm_models_per_kkr_d8r1 <- list()
...
for(kkr_val in unique(features$KKR)) {
  ...
  if(!(as.character(kkr_val) %in% names(lasso_vars_per_kkr_d8r1))) next
  
  vars <- lasso_vars_per_kkr_d8r1[[as.character(kkr_val)]]
  
  # Skip if target variable is constant or no predictors
  if(length(unique(df_kkr$D8R1)) < 2 || length(vars) == 0) next
  ...
  # Save model
  lm_models_per_kkr_d8r1[[as.character(kkr_val)]] <- lm_model
}

# Example summary for KKR = 203
summary(lm_models_per_kkr_d8r1[["203"]])

# ----- Combine regression results into one data frame -----
model_results_d8r1 <- lapply(names(lm_models_per_kkr_d8r1), function(kkr_val) {
  
  model <- lm_models_per_kkr_d8r1[[kkr_val]]
...
model_results_df_d8r1 <- do.call(rbind, model_results_d8r1)

model_results_df_d8r1 <- model_results_df_d8r1 %>%
  select(KKR, variable, estimate, std_error, t_value, p_value)

# Preview results
head(model_results_df_d8r1)


# ----- Predictions and error metrics -----
prediction_errors_d8r1 <- data.frame()

for (kkr_val in names(lm_models_per_kkr_d8r1)) {
  ...
  prediction_errors_d8r1 <- rbind(prediction_errors_d8r1, data.frame(
    KKR = kkr_val,
    n_obs = length(y_true),
    MAE = mae_val,
    RMSE = rmse_val,
