...
The variables most strongly correlated with the dependent variable are the 2-, 6-, 9-, and 12-period moving averages.
For some countries, a correlation is also observed between the average values at the municipality level and the corresponding standard deviation within the same municipality
...
```{python}
results_df = pd.DataFrame([
    {'country_en': country, 'feature': feature}
    for country, features_list in selected_features_per_country.items()
